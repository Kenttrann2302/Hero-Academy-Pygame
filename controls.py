...
class Controls_Handler():
    ...
    def update(self, actions):
        # check to see if the self.selected is true
    ...
    def navigate_menu(self, actions):
        ...
        if actions["Left"]: self.curr_index = (self.curr_index - 1) % len(self.save_file["controls"])
        if actions["Right"]: self.curr_index = (self.curr_index + 1) % len(self.save_file["controls"])
        # Handle Selection
